...
{
    //for online writer
    m_GameNeedUpdateStatusOnline = true;
    m_GameNeedUpdateStatusHost   = true;
    m_GameRehosting              = false; //флаг рехоста
    m_GameSupressDesyncWarn      = false; //сбрасываем флаг десинка, что бы вывело хоть 1 раз за катку
    ...
    for (std::vector<CGameSlot>::iterator i = m_Slots.begin(); i != m_Slots.end(); i++)
        m_GameSlotsStatuses.push_back(" ");

    m_GameSlotsStatuses[0] = "1";
    ...
    std::ofstream StatStringFile;
    StatStringFile.open("stat_string.txt", std::ios::trunc | std::ios::binary);
    if (!StatStringFile.fail())
    {
        StatStringFile << m_StatString << std::endl;
        StatStringFile.close();
    }
}
...
CBaseGame::~CBaseGame()
{
    ...
    if (m_GHost->m_TCPStatus && m_GHost->m_StatusBroadcaster != NULL)
        //m_GHost->m_StatusBroadcaster->SendName( NULL ); // игра удаляется - рассылаем про рехост
    ...
    */
    ...
    if (m_Replay && (m_GameLoading || m_GameLoaded))
    {
        ...
        memset(Time2, 0, sizeof(char) * 8);
        strftime(Time, sizeof(char) * 17, "%Y-%m-%d %H-%M", localtime(&Now));
        strftime(Time2, sizeof(char) * 8, "%Y.%m", localtime(&Now));
...
uint32_t CBaseGame::GetNumHumanPlayersFromSlots()
{
...
uint32_t CBaseGame::GetNumComps()
{
    uint32_t NumComps = 0;
    for (std::vector<CGameSlot>::iterator i = m_Slots.begin(); i != m_Slots.end(); i++)
    {
...
bool CBaseGame::Update(void *fd, void *send_fd)
{
    ...
    for (std::vector<CPotentialPlayer *>::iterator i = m_Potentials.begin(); i != m_Potentials.end();)
    {
        if ((*i)->Update(fd))
        {
            // flush the socket (e.g. in case a rejection message is std::queued)
    ...
    if (GetTime() - m_LastPingTime >= 5)
    {
        ...
        if (!m_CountDownStarted)
        {
            ...
            if (m_SaveGame)
            {
                ...
                MapHeight.push_back(0);
                m_GHost->m_UDPSocket->Broadcast(6112, m_Protocol->SEND_W3GS_GAMEINFO(m_GHost->m_TFT, m_GHost->m_LANWar3Version, UTIL_CreateByteArray(MapGameType, false), m_Map->GetMapGameFlags(), MapWidth, MapHeight, m_GameName, "beef", GetTime() - m_CreationTime, "Save\\Multiplayer\\" + m_SaveGame->GetFileNameNoPath(), m_SaveGame->GetMagicNumber(), slotstotal, slotsopen, m_HostPort, FixedHostCounter));
            }
            else
            {
                // note: the PrivateGame flag is not set when broadcasting to LAN (as you might expect)
                ...
                m_GHost->m_UDPSocket->Broadcast(6112, m_Protocol->SEND_W3GS_GAMEINFO(m_GHost->m_TFT, m_GHost->m_LANWar3Version, UTIL_CreateByteArray(MapGameType, false), m_Map->GetMapGameFlags(), m_Map->GetMapWidth(), m_Map->GetMapHeight(), m_GameName, "beef", GetTime() - m_CreationTime, m_Map->GetMapPath(), m_Map->GetMapCRC(), slotstotal, slotsopen, m_HostPort, FixedHostCounter));
            }
        }
    ...
    if (!m_GameLoading && !m_GameLoaded && GetTicks() - m_LastDownloadTicks >= 100)
    {
        ...
        for (std::vector<CGamePlayer *>::iterator i = m_Players.begin(); i != m_Players.end(); i++)
        {
            if ((*i)->GetDownloadStarted() && !(*i)->GetDownloadFinished())
            {
                ...
                // in this case any changes to the lobby are delayed by the amount of time it takes to send the std::queued data (i.e. 140 KB, which could be 30 seconds or more)
    ...
    // check authed players

    if (!m_GameLoading && !m_GameLoaded && GetTime() - m_LastAuthCheckTime >= 1) //lobby
    {
        for (std::vector<CGamePlayer *>::iterator i = m_Players.begin(); i != m_Players.end(); i++)
        {
            if (!(*i)->GetAuthenticated())
            {
                if (m_GHost->m_DB->AuthCheck((*i)->GetName())) //проверяем есть ли игрок в ДБ и с больше чем 5 игр
                {
                    (*i)->SetAuthenticated(true);
                    continue;
                }

                int timeLeftToKick = 13 - GetTime() + (*i)->GetJoinTime();

                if (timeLeftToKick == 10 || timeLeftToKick == 5)
                    SendAllChat((*i)->GetName() + " will be kicked in " + UTIL_ToString(timeLeftToKick) + " seconds...");

                if (GetTime() - (*i)->GetJoinTime() >= 13)
                {
                    (*i)->SetDeleteMe(true);
                    //(*i)->SetLeftReason(m_GHost->m_Language->WasKickedForNotSpoofChecking());
                    (*i)->SetLeftCode(PLAYERLEAVE_LOBBY);
    ...
    else if (!m_CountDownStarted && m_AutoStartPlayers != 0 && GetTime() - m_LastAutoStartTime >= 10)
    {
        StartCountDownAuto(m_GHost->m_RequireSpoofChecks);
        m_LastAutoStartTime = GetTime();
    }
    ...
    if (m_StartedDanceTime != 0 && !m_GameLoaded && !m_GameLoading)
    {
        int slot1 = 0, slot2 = 0;
        while (slot1 == slot2)
        {
        ...
        SwapSlots((unsigned char)slot1, (unsigned char)slot2);
        for (std::vector<CGameSlot>::iterator i = m_Slots.begin(); i != m_Slots.end(); i++)
        {
            (*i).SetColour(rand() % 12 + 1);
        }
    ...
    if (m_GameNeedUpdateStatusOnline == true)
    {
        ...
        //fill new online array to compare with old
        for (std::vector<CGameSlot>::iterator i = m_Slots.begin(); i != m_Slots.end(); i++)
        {
            m_GameSlotsStatuses[k] = "OUT OF GAME\n";                                   //ливер
            if ((*i).GetSlotStatus() == SLOTSTATUS_OCCUPIED && (*i).GetComputer() == 0) //условие, что игрок
            {
                for (std::vector<CGamePlayer *>::iterator p = m_Players.begin(); p != m_Players.end(); p++) //ищем имя игрока
                {
                    if (k == GetSIDFromPID((*p)->GetPID()))
                    {
                        if ((*p)->GetName() != "")
                            m_GameSlotsStatuses[k] = (*p)->GetName() + "\n";
                        else
                            m_GameSlotsStatuses[k] = "leave\n"; //ливер //не работает
                    }
                }
            }
            else if ((*i).GetSlotStatus() == SLOTSTATUS_OCCUPIED && (*i).GetComputer() == 1) //условие, что комп
            {
                m_GameSlotsStatuses[k] = "computer\n";
            }
            else if ((*i).GetSlotStatus() == SLOTSTATUS_OPEN)
            {
                m_GameSlotsStatuses[k] = "open\n";
            }
            else if ((*i).GetSlotStatus() == SLOTSTATUS_CLOSED)
            {
                m_GameSlotsStatuses[k] = "closed\n";
            }
            else
                m_GameSlotsStatuses[k] = "err\n";
            k++;
        }

        //CONSOLE_Print("updating players list");
        std::ofstream PlayersFile;
        std::ofstream StatusFile;
        PlayersFile.open("players.txt", std::ios::trunc);
        StatusFile.open("online.txt", std::ios::trunc);

        if (!StatusFile.fail())
        {
            StatusFile << m_Players.size() << std::endl;
            StatusFile << m_GameLoading << std::endl;
            StatusFile << m_GameLoaded << std::endl;
            StatusFile << m_GameRehosting << std::endl;
            StatusFile.close();
        }
        ...
        if (!PlayersFile.fail())
        {
            ...
            for (std::size_t i = 0; i < m_Slots.size(); i++)
            {
                ...
                PlayersFile << m_GameSlotsStatuses[i]; // write name or comp or open or other cases

                if (j != m_Slots.size()) //последний слот
                    if (m_Slots[j - 1].GetTeam() != m_Slots[j].GetTeam())
                        PlayersFile << std::endl; //разделяем тимы пробелами
                j++;
            }
    ...
    {
        //CONSOLE_Print("updating host status");
        std::ofstream StatusFile;
        StatusFile.open("online.txt", std::ios::trunc);
        if (!StatusFile.fail())
        {
    ...
    */
    ...
    if (m_GHost->m_gameoverminpercent != 0 && (!m_GameLoading && m_GameLoaded))
        if (remainingpercent < m_GHost->m_gameoverminpercent)
            lessthanminpercent = true;
    if (lessthanminpercent && m_GameOverTime == 0)
    {
        CONSOLE_Print("[GAME: " + m_GameName + "] gameover timer started (less than " + UTIL_ToString(m_GHost->m_gameoverminpercent) + "% )");
        SendAllChat("Game over in 60 seconds, " + UTIL_ToString((int)remainingpercent) + "% remaining ( < " + UTIL_ToString(m_GHost->m_gameoverminpercent) + "% ) ");
        m_GameOverTime = GetTime();
    }
...
void CBaseGame::UpdatePost(void *send_fd)
{
    ...
    // in reality since we're std::queueing actions it might not make a big difference but oh well
...
void CBaseGame::EventPlayerDeleted(CGamePlayer *player)
{
    ...
    // remove any std::queued spoofcheck messages for this player
...
void CBaseGame::EventPlayerJoined(CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer)
{
    ...
    {
        int num;
        std::vector<std::string> ApprovedLocations;
        ...
        for (std::size_t i = 0; i < m_GHost->m_ApprovedCountries.length(); i += 2)
...
void CBaseGame::EventPlayerJoinedWithScore(CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer, double score)
{
    ...
    if (SID == 255)
    {
        ...
        else if (m_GHost->m_MatchMakingMethod == 1)
        {
            ...
            for (std::vector<CGamePlayer *>::iterator i = m_Players.begin(); i != m_Players.end(); i++)
            {
                if (!FurthestPlayer || (*i)->GetScore() < -99999.0 || std::abs((*i)->GetScore() - AverageScore) > std::abs(FurthestPlayer->GetScore() - AverageScore))
                    FurthestPlayer = *i;
            }
...
void CBaseGame::EventPlayerLeft(CGamePlayer *player, uint32_t reason)
{
    ...
    if (reason == PLAYERLEAVE_GPROXY)
    {
        ...
        // SendAllChat( player->GetName( ) + ": GPROXY HUEHUEHUE");
        return;
    }
    else
...
{
    if (m_GameSupressDesyncWarn && (m_GHost->m_DesyncKick == false))
        return; //если отключен кик при десинке и не нужно вывести сообщение о десинке - выходим
    ...
    [[maybe_unused]] bool AddToReplay = true;
    ...
    for (std::vector<CGamePlayer *>::iterator i = m_Players.begin(); i != m_Players.end(); i++)
    {
        if (!(*i)->GetDeleteMe() && (*i)->GetCheckSums()->front() != FirstCheckSum)
        {
            ...
            m_GameSupressDesyncWarn = true; //что бы не проверять и не спамить десинком
            if (m_GHost->m_DesyncKick == false)
                return; //если кик на десинке вырублен - выходим, не кикаем далее
...
void CBaseGame::EventPlayerChatToHost(CGamePlayer *player, CIncomingChatPlayer *chatPlayer)
{
    ...
    std::string tmp_message = chatPlayer->GetChatMessage();

    //replace_with(tmp_message, "hui", "Reimu");
    //replace_with(tmp_message, "Hui", "Reimu");
    //replace_with(tmp_message, "HUI", "Reimu");
    /*replace_with(tmp_message, "penir", "Reimu");
    replace_with(tmp_message, "Penir", "Reimu");
    replace_with(tmp_message, "PENIR", "Reimu");
    replace_with(tmp_message, "penis", "Reimu");
    replace_with(tmp_message, "Penis", "Reimu");
    replace_with(tmp_message, "PENIS", "Reimu");
    //replace_with(tmp_message, "KACHKA", "Reimu");
    //replace_with(tmp_message, "Kachka", "Reimu");
    //replace_with(tmp_message, "kachka", "Reimu");
    replace_with(tmp_message, "dots", "DODS");
    replace_with(tmp_message, "DOTS", "DODS");
    replace_with(tmp_message, "DoTS", "DODS");
    replace_with(tmp_message, "Dots", "DODS");
    replace_with(tmp_message, "Dota", "DODA");
    replace_with(tmp_message, "dota", "DODA");
    replace_with(tmp_message, "DOTA", "DODA");
    replace_with(tmp_message, "dotu", "DODU");
    replace_with(tmp_message, "DOTU", "DODU");*/
    //for(int i = 1; i < 10; i++)
    //	replace_with(tmp_message, m_GHost->m_Language->Hui(i), "Reimu");
    chatPlayer->SetMessage(tmp_message);
    ...
    if (chatPlayer->GetFromPID() == player->GetPID())
    {
        if (chatPlayer->GetType() == CIncomingChatPlayer::CTH_MESSAGE || chatPlayer->GetType() == CIncomingChatPlayer::CTH_MESSAGEEXTRA)
        {
            ...
            std::string Message = chatPlayer->GetChatMessage();
            ...
            if (Relay)
                Send(chatPlayer->GetToPIDs(), m_Protocol->SEND_W3GS_CHAT_FROM_HOST(chatPlayer->GetFromPID(), chatPlayer->GetToPIDs(), chatPlayer->GetFlag(), chatPlayer->GetExtraFlags(), chatPlayer->GetChatMessage()));
        }
...
void CBaseGame::EventGameStarted()
{
    ...
    if (m_Map->GetMapHCLFromGameName() || !m_HCLOverride)
    {
        ...
        std::vector<std::string> ValidModes = UTIL_Tokenize(m_Map->GetMapHCLValidModes(), ' ');
        std::string::size_type loc;

        for (std::vector<std::string>::iterator i = ValidModes.begin(); i != ValidModes.end(); i++)
        {
...
void CBaseGame::OpenSlot(unsigned char SID, bool kick)
{
    if (SID < m_Slots.size())
    {
        ...
        uint32_t oldOpenSlotsNum = GetSlotsOpen();

        m_Slots[SID] = CGameSlot(0, 255, SLOTSTATUS_OPEN, 0, Slot.GetTeam(), Slot.GetColour(), Slot.GetRace());
        SendAllSlotInfo();

        if (!Player)                               // не рассылаем если слот был  игроком, т.к. для игроков отдельная рассылка EventPlayerDeleted
            if (GetSlotsOpen() != oldOpenSlotsNum) // поменялось количество открытых слотов - рассылаем
            {
                if (m_GHost->m_TCPStatus && m_GHost->m_StatusBroadcaster != NULL)
                    //m_GHost->m_StatusBroadcaster->SendNump(m_GHost->GetGame()); // рассылаем количество игроков
                    m_GHost->m_StatusBroadcaster->SendGame(m_GHost->GetGame());
...
void CBaseGame::ShuffleSlots()
{
    ...
    //смотрим где самый первый слот с игроком - с админскими правами
    int AdminSlot = 99;
    for (std::vector<CGameSlot>::iterator i = m_Slots.begin(); i != m_Slots.end(); i++)
    {
        if ((*i).GetSlotStatus() == SLOTSTATUS_OCCUPIED && (*i).GetComputer() == 0 && (*i).GetTeam() != 12)
        {
            AdminSlot = i - m_Slots.begin();
            break;
        }
    }
    ...
    for (std::vector<CGameSlot>::iterator i = m_Slots.begin(); i != m_Slots.end(); i++)
    {
        //скипаем обсов, не шаффлим их
        if (std::find(ObserverSlots.begin(), ObserverSlots.end(), (i - m_Slots.begin())) != ObserverSlots.end())
            continue;
        else
        {
            if ((*i).GetSlotStatus() == SLOTSTATUS_OCCUPIED && (*i).GetComputer() == 0 && (*i).GetTeam() != 12 && AdminSlot != (i - m_Slots.begin())) //тут в конце проверка, что не админский слот
    ...
    if (m_Map->GetMapOptions() & MAPOPT_FIXEDPLAYERSETTINGS)
    {
        ...
        std::shuffle(SIDs.begin(), SIDs.end(), std::default_random_engine(seed));

        // now put the PlayerSlots std::vector in the same order as the SIDs vector
    ...
    else
    {
        ...
        std::shuffle(PlayerSlots.begin(), PlayerSlots.end(), std::default_random_engine(seed));
    }
...
void CBaseGame::AddTmpRootAdmin(std::string name)
{
    transform(name.begin(), name.end(), name.begin(), (int (*)(int))tolower);

    if (m_TmpRootAdmins == "")
        m_TmpRootAdmins = name;
    else
        m_TmpRootAdmins.append(" " + name);
}

bool CBaseGame::IsTmpRootAdmin(std::string name)
{
    transform(name.begin(), name.end(), name.begin(), (int (*)(int))tolower);

    if (m_TmpRootAdmins.find(name) != std::string::npos)
        return true;
