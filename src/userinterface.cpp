...
CCurses::CCurses(int nTermWidth, int nTermHeight, bool nSplitView, int nListType)
{
    ...
    m_Buffers[B_INPUT]->push_back(std::pair<std::string, int>("", 0));
...
void CCurses::AddTab(std::string nName, TabType nType, uint32_t nId, BufferType nBufferType, WindowType nWindowType)
{
    ...
    for (std::vector<STabData>::iterator i = m_TabData.begin(); i != m_TabData.end(); i++)
    {
...
void CCurses::UpdateWindow(WindowType type)
{
    ...
    if (!m_TabData.empty() && m_Buffers[m_TabData[m_SelectedTab].bufferType]->size() > (std::size_t)LINES / 2)
    {
...
void CCurses::CompileFriends()
{
    ...
    for (uint32_t i = 0; i < 10; ++i)
    {
        ...
        {
            m_Buffers[B_FRIENDS]->push_back(std::pair<std::string, int>("[" + m_RealmData[i].RealmAlias + "]", 1));

            for (uint32_t j = 0; j < m_RealmData[i].Friends.size(); ++j)
            {
                m_Buffers[B_FRIENDS]->push_back(std::pair<std::string, int>(" " + m_RealmData[i].Friends[j].first, m_RealmData[i].Friends[j].second));
            }

            m_Buffers[B_FRIENDS]->push_back(std::pair<std::string, int>("\n", 1));
        }
    }
}
...
void CCurses::CompileClan()
{
    ...
    for (uint32_t i = 0; i < 10; ++i)
    {
        ...
        {
            m_Buffers[B_CLAN]->push_back(std::pair<std::string, int>("[" + m_RealmData[i].RealmAlias + "]", 1));

            for (uint32_t j = 0; j < m_RealmData[i].Clan.size(); ++j)
            {
                m_Buffers[B_CLAN]->push_back(std::pair<std::string, int>(" " + m_RealmData[i].Clan[j].first, m_RealmData[i].Clan[j].second));
            }

            m_Buffers[B_CLAN]->push_back(std::pair<std::string, int>("\n", 1));
        }
    }
}
...
void CCurses::CompileBans()
{
    ...
    for (uint32_t i = 0; i < 10; ++i)
    {
        ...
        {
            m_Buffers[B_BANS]->push_back(std::pair<std::string, int>("[" + m_RealmData[i].RealmAlias + "]", 1));

            for (uint32_t j = 0; j < m_RealmData[i].Bans.size(); ++j)
            {
                m_Buffers[B_BANS]->push_back(std::pair<std::string, int>(" " + m_RealmData[i].Bans[j].first, m_RealmData[i].Bans[j].second));
            }

            m_Buffers[B_BANS]->push_back(std::pair<std::string, int>("\n", 1));
        }
    }
}
...
void CCurses::CompileAdmins()
{
    ...
    for (uint32_t i = 0; i < 10; ++i)
    {
        ...
        {
            m_Buffers[B_ADMINS]->push_back(std::pair<std::string, int>("[" + m_RealmData[i].RealmAlias + "]", 1));

            for (uint32_t j = 0; j < m_RealmData[i].Admins.size(); ++j)
            {
                m_Buffers[B_ADMINS]->push_back(std::pair<std::string, int>(" " + m_RealmData[i].Admins[j].first, m_RealmData[i].Admins[j].second));
            }

            m_Buffers[B_ADMINS]->push_back(std::pair<std::string, int>("\n", 1));
        }
    }
}
...
void CCurses::CompileGames()
{
    ...
    if (m_GHost->m_Games.empty() && m_GHost->m_CurrentGame)
        m_Buffers[B_GAMES]->push_back(std::pair<std::string, int>("0. " + m_GHost->m_CurrentGame->GetDescription() + "\n", 0));
    else
    {
        ...
        {
            m_Buffers[B_GAMES]->push_back(std::pair<std::string, int>(UTIL_ToString(i + 1) + ". " + m_GHost->m_Games[i]->GetDescription() + "\n", 0));
        }
    }
}
...
void CCurses::SetAttribute(SWindowData &data, std::string message, int flag, BufferType type, bool on)
{
    ...
    case B_REALM:
        ...
        case 0:
            attribute = A_WHITE;
            break; // NORMAL, GAME, MAP, CONFIG, UTIL
        case 1:
            attribute = A_CYAN;
            break; // INFO, EMOTE
        case 2:
    ...
    case B_FRIENDS:
        ...
        case 3:
            attribute = A_GREEN;
            break; // IN PUBLIC GAME
        case 4:
            attribute = A_YELLOW;
            break; // IN PRIVATE GAME; NOT MUTUAL
        case 5:
            attribute = A_CYAN;
            break; // IN PRIVATE GAME; MUTUAL
        }
        break;
    case B_ADMINS:
        if (flag == 2)
...
{
    message                          = UTIL_UTF8ToLatin1(message);
    std::pair<std::string, int> temp = std::pair<std::string, int>(message, 0);
    if (toMainBuffer)
    {
...
void CCurses::ScrollDown()
{
    ...
    {
        m_WindowData[W_CHANNEL].Scroll          = (uint32_t)m_WindowData[W_CHANNEL].Scroll < m_Buffers[B_CHANNEL]->size() ? m_WindowData[W_CHANNEL].Scroll + SCROLL_VALUE : m_WindowData[W_CHANNEL].Scroll;
    ...
    else
    {
        ...
        {
        case T_MAIN:
            m_WindowData[W_FULL2].Scroll          = (uint32_t)m_WindowData[W_FULL2].Scroll < m_Buffers[m_TabData[m_SelectedTab].bufferType]->size() ? m_WindowData[W_FULL2].Scroll + SCROLL_VALUE : m_WindowData[W_FULL2].Scroll;
            m_WindowData[W_FULL2].IsWindowChanged = true;
            break;
        case T_REALM:
            if (m_SplitView)
            {
                if (exY < LINES / 2)
                {
                    m_WindowData[W_UPPER].Scroll          = (uint32_t)m_WindowData[W_UPPER].Scroll < m_Buffers[B_MAIN]->size() ? m_WindowData[W_UPPER].Scroll + SCROLL_VALUE : m_WindowData[W_UPPER].Scroll;
                    m_WindowData[W_UPPER].IsWindowChanged = true;
                }
                else
                {
                    m_WindowData[W_LOWER].Scroll          = (uint32_t)m_WindowData[W_LOWER].Scroll < m_Buffers[m_TabData[m_SelectedTab].bufferType]->size() ? m_WindowData[W_LOWER].Scroll + SCROLL_VALUE : m_WindowData[W_LOWER].Scroll;
                    m_WindowData[W_LOWER].IsWindowChanged = true;
                }
            }
            else
            {
                m_WindowData[W_FULL].Scroll          = (uint32_t)m_WindowData[W_FULL].Scroll < m_Buffers[m_TabData[m_SelectedTab].bufferType]->size() ? m_WindowData[W_FULL].Scroll + SCROLL_VALUE : m_WindowData[W_FULL].Scroll;
                m_WindowData[W_FULL].IsWindowChanged = true;
            }
            break;
        default:
...
void CCurses::ScrollUp()
{
    if (exX > COLS - 23 && m_TabData[m_SelectedTab].type == T_REALM)
    {
        m_WindowData[W_CHANNEL].Scroll          = m_WindowData[W_CHANNEL].Scroll - SCROLL_VALUE > LINES ? m_WindowData[W_CHANNEL].Scroll - SCROLL_VALUE : LINES - 4;
        m_WindowData[W_CHANNEL].IsWindowChanged = true;
    }
    else
    {
        ...
        {
        case T_MAIN:
            m_WindowData[W_FULL2].Scroll          = m_WindowData[W_FULL2].Scroll - SCROLL_VALUE > LINES ? m_WindowData[W_FULL2].Scroll - SCROLL_VALUE : LINES - 4;
            m_WindowData[W_FULL2].IsWindowChanged = true;
            break;
        case T_REALM:
            if (m_SplitView)
            {
                if (exY < LINES / 2)
                {
                    m_WindowData[W_UPPER].Scroll          = m_WindowData[W_UPPER].Scroll - SCROLL_VALUE > LINES / 2 ? m_WindowData[W_UPPER].Scroll - SCROLL_VALUE : (LINES - 4) / 2;
                    m_WindowData[W_UPPER].IsWindowChanged = true;
                }
                else
                {
                    m_WindowData[W_LOWER].Scroll          = m_WindowData[W_LOWER].Scroll - SCROLL_VALUE > LINES / 2 ? m_WindowData[W_LOWER].Scroll - SCROLL_VALUE : (LINES - 4) / 2;
                    m_WindowData[W_LOWER].IsWindowChanged = true;
                }
            }
            else
            {
                m_WindowData[W_FULL].Scroll          = m_WindowData[W_FULL].Scroll - SCROLL_VALUE > LINES ? m_WindowData[W_FULL].Scroll - SCROLL_VALUE : LINES - 4;
                m_WindowData[W_FULL].IsWindowChanged = true;
            }
            break;
        default:
    ...
    while (c != ERR && Connected)
    {
        ...
        if (m_Buffers[B_INPUT]->size() > 1)
        {
            if (c == KEY_UP)
            {
                ...
                if (m_SelectedInput != m_Buffers[B_INPUT]->size() - 1)
                {
                    ...
                    m_Buffers[B_INPUT]->push_back(std::pair<std::string, int>(m_InputBuffer, 0));
                }
                return false;
            }
            else if (c == KEY_DOWN)
...
#endif
        {
            ...
            else if (m_TabData[m_SelectedTab].type == T_REALM)
            {
                ...
                return false; // don't clear the m_InputBuffer
...
uint32_t CCurses::GetMessageFlag(std::string &message)
{
    ...
    {
        if (message.compare(1, 4, "info") == 0)
            return 1;
        else if (message.compare(1, 4, "bnet") == 0)
            return 4;
        else if (message.compare(1, 5, "ghost") == 0)
            return 2;
        else if (message.compare(1, 5, "bnlsc") == 0)
            return 4;
        else if (message.compare(1, 5, "error") == 0)
            return 3;
        else if (message.compare(1, 5, "emote") == 0)
            return 1;
        else if (message.compare(1, 7, "whisper") == 0)
            return 5;
    }
...
void CCurses::AddChannelUser(std::string name, uint32_t realmId, int flag)
{
    ...
    m_RealmData[realmId].ChannelUsers.push_back(std::pair<std::string, int>(name, flag));
    m_WindowData[W_CHANNEL].IsWindowChanged = true;
...
void CCurses::UpdateCustomLists(BufferType type)
{
    if (m_GHost)
    {
        for (uint32_t i = 0; i < m_GHost->m_BNETs.size(); ++i)
        {
            ...
            {
            case B_FRIENDS:
                m_RealmData[i].Friends = m_GHost->m_BNETs[i]->GetFriends();
            case B_CLAN:
                m_RealmData[i].Clan = m_GHost->m_BNETs[i]->GetClan();
            case B_BANS:
                m_RealmData[i].Bans = m_GHost->m_BNETs[i]->GetBans();
            case B_ADMINS:
                m_RealmData[i].Admins = m_GHost->m_BNETs[i]->GetAdmins();
            default:
                break;
            }
        }
