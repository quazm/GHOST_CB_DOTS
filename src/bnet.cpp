...
{
    // todotodo: append std::filesystem::path seperator to Warcraft3Path if needed
...
std::vector<std::pair<std::string, int>> CBNET::GetFriends()
{
    std::vector<std::pair<std::string, int>> result;

    for (std::vector<CIncomingFriendList *>::iterator i = m_Friends.begin(); i != m_Friends.end(); i++)
        result.push_back(std::pair<std::string, int>((*i)->GetAccount(), (*i)->GetArea()));

    return result;
}

std::vector<std::pair<std::string, int>> CBNET::GetClan()
{
    std::vector<std::pair<std::string, int>> result;

    int k = 0;
    for (std::vector<CIncomingClanList *>::iterator i = m_Clans.begin(); i != m_Clans.end() && k < 50; i++, k++)
        result.push_back(std::pair<std::string, int>((*i)->GetName(), (*i)->GetRawStatus()));

    return result;
}

std::vector<std::pair<std::string, int>> CBNET::GetBans()
{
    std::vector<std::pair<std::string, int>> result;

    int k = 0;
    for (std::vector<CDBBan *>::iterator i = m_Bans.begin(); i != m_Bans.end() && k < 100; i++, k++)
        result.push_back(std::pair<std::string, int>((*i)->GetName(), 0));

    return result;
}

std::vector<std::pair<std::string, int>> CBNET::GetAdmins()
{
    std::vector<std::pair<std::string, int>> result;

    std::vector<std::string> roots = UTIL_Tokenize(m_RootAdmin, ' ');

    for (std::vector<std::string>::iterator i = roots.begin(); i != roots.end(); i++)
        result.push_back(std::pair<std::string, int>(*i, 2));

    for (std::vector<std::string>::iterator i = m_Admins.begin(); i != m_Admins.end(); i++)
        result.push_back(std::pair<std::string, int>(*i, 0));
...
void CBNET::ProcessChatEvent(CIncomingChatEvent *chatEvent)
{
    ...
    std::string Message                    = chatEvent->GetChatMessage();
    ...
    if (Event == CBNETProtocol::EID_WHISPER || Event == CBNETProtocol::EID_TALK || Event == 29)
    {
        ...
        if (Message == "?trigger" && (IsAdmin(User) || IsRootAdmin(User) || (m_PublicCommands && m_OutPackets.size() <= 3)))
            QueueChatCommand(m_GHost->m_Language->CommandTrigger(std::string(1, m_CommandTrigger)), User, Whisper, WhisperResponses);
        else if (!Message.empty() && Message[0] == m_CommandTrigger)
        {
                ...
                ******************/
                ...
                if (Command == "announce" && m_GHost->m_CurrentGame && !m_GHost->m_CurrentGame->GetCountDownStarted())
                {
                    if (Payload.empty() || Payload == "off")
                    {
                        ...
                        m_GHost->m_CurrentGame->SetAnnounce(0, std::string());
                    }
                    else
                    {
                ...
                if ((Command == "saygames" || Command == "s") && !Payload.empty())
                {
                    if (IsRootAdmin(User))
                    {
                        if (m_GHost->m_CurrentGame)
            ...
            *********************/
            ...
            // e.g. when several users join a game at the same time and cause multiple /whois messages to be std::queued at once
...
void CBNET::QueueChatCommand(std::string chatCommand, bool hidden)
{
    ...
    if (m_LoggedIn)
    {
        ...
        else
        {
            ...
            if (chatCommand.size() > 3)
            {
                ...
                else if (chatCommand.substr(0, 9) == "/whisper ")
                {
                    whisper = true;
                    r       = 9;
                }
                else if (chatCommand.substr(0, 5) == "/f m ")
                {
                    whisper = true;
                    r       = 5;
                }
                else if (chatCommand.substr(0, 7) == "/f msg ")
                {
                    whisper = true;
                    r       = 7;
                }
                else if (chatCommand.substr(0, 11) == "/friends m ")
                {
                    whisper = true;
                    r       = 11;
                }
                else if (chatCommand.substr(0, 13) == "/friends msg ")
                {
                    whisper = true;
                    r       = 13;
                }
            ...
            if (whisper)
            {
                if (r == 3 || r == 9)
                {
                    ...
                    if (nameEndpos != -1)
                    {
                        std::string target = chatCommand.substr(r, nameEndpos - r);
                    }
                }
                else
                    CONSOLE_Print("[WHISPERED: " + m_ServerAlias + "] [Friends] " + chatCommand.substr(r, chatCommand.length()), GetRealmId(), false);
            }
            else if (!hidden)
                CONSOLE_Print("[" + m_UserName + "] " + chatCommand, GetRealmId(), false);
...
void CBNET::QueueChatCommand(std::string chatCommand, std::string user, bool whisper, bool whisperresponses)
{
    if (chatCommand.empty())
        return;
...
bool CBNET::IsRootAdmin(std::string name)
{
    ...
    if (m_RootAdmin.find(name) != std::string::npos)
        return true;

    return false;
}

bool CBNET::IsLANRootAdmin(std::string name)
{
    // m_LANRootAdmin was already transformed to lower case in the constructor

    transform(name.begin(), name.end(), name.begin(), (int (*)(int))tolower);

    if (m_LANRootAdmin.find(name) != std::string::npos)
        return true;
...
//void CBNET::AddTmpRootAdmin(std::string name)
//{
//	transform(name.begin(), name.end(), name.begin(), (int(*)(int))tolower);
//	m_RootAdmin.append(" " + name);
//}
...
void CBNET::HiddenGhostCommand(std::string Message)
{
