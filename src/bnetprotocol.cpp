...
BYTEARRAY CBNETProtocol::SEND_SID_ENTERCHAT()
{
    ...
    packet.push_back(0);                    // Stat std::string is NULL on CDKEY'd products
    AssignLength(packet);
...
std::map author: (mask 0x00006000) can be combined
*0x00002000 - Blizzard
 0x00004000 - Custom
Battle type: (mask 0x00018000) cant be combined
 0x00000000 - Battle
*0x00010000 - Scenario
std::map size: (mask 0x000E0000) can be combined with 2 nearest values
 0x00020000 - Small
 0x00040000 - Medium
*0x00080000 - Huge
...
*/
    ...
    if (mapGameType.size() == 4 && mapFlags.size() == 4 && mapWidth.size() == 2 && mapHeight.size() == 2 && !gameName.empty() && !hostName.empty() && !mapPath.empty() && mapCRC.size() == 4 && mapSHA1.size() == 20 && StatString.size() < 128 && HostCounterString.size() == 8)
    {
        ...
        packet.push_back(0);                                        // Stat std::string null terminator (the stat std::string is encoded to remove all even numbers i.e. zeros)
        AssignLength(packet);
    }
    else
...
CIncomingGameHost::CIncomingGameHost(BYTEARRAY &nIP, uint16_t nPort, std::string nGameName, BYTEARRAY &nHostCounter)
{
    m_IP          = nIP;
    m_Port        = nPort;
...
CIncomingChatEvent::CIncomingChatEvent(CBNETProtocol::IncomingChatEvent nChatEvent, uint32_t nUserFlags, uint32_t nPing, std::string nUser, std::string nMessage)
{
...
CIncomingFriendList::CIncomingFriendList(std::string nAccount, unsigned char nStatus, unsigned char nArea, std::string nLocation)
{
    m_Account  = nAccount;
    m_Status   = nStatus;
    m_Area     = nArea;
...
CIncomingClanList::CIncomingClanList(std::string nName, unsigned char nRank, unsigned char nStatus)
{
    m_Name   = nName;
    m_Rank   = nRank;
    m_Status = nStatus;
