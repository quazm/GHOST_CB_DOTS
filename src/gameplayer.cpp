...
{
    // todotodo: properly copy std::queued packets to the new player, this just discards them
...
void CGamePlayer::ProcessPackets()
{
    ...
    [[maybe_unused]] bool HasMap    = false;
    uint32_t CheckSum               = 0;
    uint32_t Pong                   = 0;
        ...
        if (Packet->GetPacketType() == W3GS_HEADER_CONSTANT)
        {
            ...
            case CGameProtocol::W3GS_PONG_TO_HOST:
                ...
                if (Pong != 1)
                {
                    ...
                    // this statement also gives the player a 5 second grace period after downloading the map to allow std::queued (i.e. delayed) ping packets to be ignored
