...
private:
    ...
    std::queue<BYTEARRAY> m_LoadInGameData; // std::queued data to be sent when the player finishes loading when using "load in game"
...
public:
    ...
    bool GetAuthenticated() { return m_Authenticated; }
    ...
    void SetAuthenticated(bool nAuthenticated) { m_Authenticated = nAuthenticated; }
