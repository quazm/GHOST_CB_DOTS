...
CIncomingGarenaUser *CGCBIProtocol::RECEIVE_GCBI_INIT(BYTEARRAY data)
{
    ...
    // 2 bytes					-> country std::string from Garena
    ...
    if (ValidateLength(data) && data.size() == 22)
    {
        ...
        return new CIncomingGarenaUser(UTIL_ByteArrayToUInt32(IP, true),
                                       ...
                                       std::string(Country.begin(), Country.end()));
    }
...
CIncomingGarenaUser::CIncomingGarenaUser(uint32_t nIP, uint32_t nUserID, uint32_t nRoomID, uint32_t nUserExp, std::string nCountryCode) : m_IP(nIP), m_UserID(nUserID), m_RoomID(nRoomID), m_UserExp(nUserExp), m_CountryCode(nCountryCode)
{
}
