...
protected:
    //
    bool m_GameNeedUpdateStatusOnline; //флаг, указывающий, нужно ли обновлять файл  с онлайн статой
    bool m_GameNeedUpdateStatusHost;   //флаг, указывающий, нужно ли обновлять файл статуса
    std::vector<std::string> m_GameSlotsStatuses;
    bool m_GameRehosting;         //флаг, указывающий, что игра рехостит в данный момент
    bool m_GameSupressDesyncWarn; //флаг, не писать сообщение о десинке
    std::string m_ObserverSlots;  //слоты, в которые нельзя свапать
    std::string m_TmpRootAdmins;  //временные админы
    //
...
public:
    ...
    virtual std::vector<std::string> GetGameSlotsStatuses() { return m_GameSlotsStatuses; }
    virtual bool GetGameRehostingFlag() { return m_GameRehosting; }
    virtual bool GetGameLoadingFlag() { return m_GameLoading; }
    virtual bool GetGameLoadedFlag() { return m_GameLoaded; }
    virtual bool GetGameNeedUpdateStatusOnline() { return m_GameNeedUpdateStatusOnline; }
    virtual bool GetGameNeedUpdateStatusHost() { return m_GameNeedUpdateStatusHost; }
    ...
    virtual std::vector<CGameSlot> GetSlots() { return m_Slots; }
    virtual std::vector<CGamePlayer *> GetPlayers() { return m_Players; }
    virtual std::string GetStatString() { return m_StatString; }
    virtual uint32_t GetFakePlayerPID() { return m_FakePlayerPID; }

    //tmp root function
    virtual std::string GetTmpRootAdmins() { return m_TmpRootAdmins; }
    virtual void AddTmpRootAdmin(std::string name);
    virtual bool IsTmpRootAdmin(std::string name);
    //---
    ...
    uint32_t GetNumHumanPlayersFromSlots();
    virtual uint32_t GetNumHumanPlayers();
    virtual uint32_t GetNumComps();
