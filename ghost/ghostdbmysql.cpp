...
bool CGHostDBMySQL::AuthCheck(string user)
{
	...
	if (!(Connection = mysql_init(NULL)))
	{
		...
		CONSOLE_Print(string("error: ") + mysql_error(Connection));
		return 0;
	}
	...
	string EscUser = MySQLEscapeString(Connection, user);

	//get name from alias if any
	string Query = "SELECT * FROM players_aliases WHERE alias1 = \"" + EscUser + "\" OR alias2 = \"" + EscUser + "\" OR alias3 = \"" + EscUser + "\" OR alias4 = \"" + EscUser + "\"";
