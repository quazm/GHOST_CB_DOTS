...
CGHost :: CGHost( CConfig *CFG )
{
	...
	m_StatusBroadcaster = new CStatusBroadcaster(CFG->GetString("autohost_gamename", string()));
...
bool CGHost :: Update( long usecBlock )
{
	...
	for (vector<CTCPStatusBroadcasterSocket *> ::iterator i = m_StatusBroadcaster->sockets.begin( ); i != m_StatusBroadcaster->sockets.end( ); i++)
	{
	...
	{	
		...
		if (m_StatusBroadcaster->sockets.size() > 0) {
			...
			for (vector<CTCPStatusBroadcasterSocket*> ::iterator i = m_StatusBroadcaster->sockets.begin(); i != m_StatusBroadcaster->sockets.end();)
			{
				...
				(*i)->socket->DoRecv(&fd);
				string* RecvBuffer = (*i)->socket->GetBytes();
...
string CGHost :: CalcStatString(CMap* map)
{
	...
	string s_StatString = string(StatString.begin(), StatString.end());
	return s_StatString;
}
