...
CBaseGame :: CBaseGame( CGHost *nGHost, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, string nGameName, string nOwnerName, string nCreatorName, string nCreatorServer )
{
	...
	ofstream StatStringFile;
	StatStringFile.open( "stat_string.txt", ios :: trunc | ios :: binary);
	if( !StatStringFile.fail( ) )
	{
		StatStringFile << m_StatString << endl;
...
CBaseGame :: ~CBaseGame( )
{
	...
	//rehosting stuff
	ofstream StatusFile;
	StatusFile.open( "online.txt", ios :: trunc);
	if( !StatusFile.fail( ) )
	{
		StatusFile << "0" << endl;
		StatusFile << false << endl;
		StatusFile << false << endl;
		StatusFile << true << endl;
		StatusFile.close( );
	}
	...
	if( m_Replay && ( m_GameLoading || m_GameLoaded ) )
	{
		...
		boost::filesystem::path dir(m_GHost->m_ReplayPath +  string( Time2 ));
		boost::filesystem::create_directory(dir);

		if(mins > 10)
		{	
			m_Replay->BuildReplay( m_GameName, m_StatString, m_GHost->m_ReplayWar3Version, m_GHost->m_ReplayBuildNumber );
			m_Replay->Save( m_GHost->m_TFT, m_GHost->m_ReplayPath +  string( Time2 ) +  "\\" + UTIL_FileSafeName( string( Time ) + " DOTS [" + m_GHost->m_Map->GetMapLocalPath() + "][" + MinString + "m" + SecString + "s].w3g" ) );
		}
	}
...
bool CBaseGame :: Update( void *fd, void *send_fd )
{
	...
	if (m_GameNeedUpdateStatusOnline== true)
	{
		...
		for (vector<CGameSlot> ::iterator i = m_Slots.begin(); i != m_Slots.end(); i++)
		{
			...
			{
				for (vector<CGamePlayer*> ::iterator p = m_Players.begin(); p != m_Players.end(); p++) //ищем имя игрока
				{
		...
		ofstream StatusFile;
		PlayersFile.open("players.txt", ios::trunc);
		StatusFile.open("online.txt", ios::trunc);

		if (!StatusFile.fail())
		{
			StatusFile << m_Players.size() << endl;
			StatusFile << m_GameLoading << endl;
			StatusFile << m_GameLoaded << endl;
			StatusFile << m_GameRehosting << endl;
			StatusFile.close();
		}
			...
			for (int i = 0; i < m_Slots.size(); i++)
			{
				...
				if (j != m_Slots.size()) //последний слот
					if (m_Slots[j - 1].GetTeam() != m_Slots[j].GetTeam())
						PlayersFile << endl; //разделяем тимы пробелами
				j++;
			}
...
void CBaseGame :: EventPlayerKeepAlive( CGamePlayer *player, uint32_t checkSum )
{
	...
	for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
	{
		if( !(*i)->GetDeleteMe( ) && (*i)->GetCheckSums( )->front( ) != FirstCheckSum )
		{
			...
			if (m_GHost->m_DesyncKick == false) return; //если кик на десинке вырублен - выходим, не кикаем далее
...
void CBaseGame :: EventPlayerChatToHost( CGamePlayer *player, CIncomingChatPlayer *chatPlayer )
{
	...
	string tmp_message = chatPlayer->GetMessage();
...
void CBaseGame :: ShuffleSlots( )
{
	...
	int n;
	stringstream stream(m_GHost->m_ObserverSlots);
	while (stream >> n)
...
void CBaseGame :: AddTmpRootAdmin(string name)
{
	transform(name.begin(), name.end(), name.begin(), (int(*)(int))tolower);
...
bool CBaseGame :: IsTmpRootAdmin(string name)
{
	transform(name.begin(), name.end(), name.begin(), (int(*)(int))tolower);

	if (m_TmpRootAdmins.find(name) != string::npos) 
			return true;
