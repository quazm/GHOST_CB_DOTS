...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	if( (player->GetSpoofed( ) || spoofed) && ( AdminCheck || RootAdminCheck || IsOwner( User ) ) )
	{
		...
		if( !m_Locked || RootAdminCheck || IsOwner( User ) )
		{
			...
			******************/
			...
			else if(Command == "priv") SendAllChat( "anus sebe !priv" );
			...
			else if(Command == "pub") SendAllChat( "anus sebe !pub");
			...
			else if(Command == "startn" && m_Players.size() < 2) SendAllChat("Need one more player for start");
			else if(Command == "startn" && !RootAdminCheck) SendAllChat("Need root rights for !startn");
	...
	*********************/
	...
	{
		HideCommand = true;
		SendChat(player, "Desync kick: " + string(m_GHost->m_DesyncKick ? "ON (will kick on desync)" : "OFF (will not kick on desync)"));	
	}
